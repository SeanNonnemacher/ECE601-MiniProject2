...
hidden_layer_2 = 256
hidden_layer_3 = 128
output_lay = 10   # output layer (0-9 digits)

X = tf.placeholder("float", [None, input_layer])
Y = tf.placeholder("float", [None, output_lay])
keep_prob = tf.placeholder(tf.float32) 

weights = {
    'w1': tf.Variable(tf.truncated_normal([input_layer, hidden_layer_1], stddev=0.1)),
    ...
    'out': tf.Variable(tf.truncated_normal([hidden_layer_3, output_lay], stddev=0.1)),
}
...
biases = {
    ...
    'out': tf.Variable(tf.constant(0.1, shape=[output_lay]))
}
